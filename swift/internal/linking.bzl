...
load(":utils.bzl", "objc_provider_framework_name")
...
def register_libraries_to_link(
        ...
        objects,
        swift_toolchain):
    """Declares files for the requested libraries and links them.
...
def register_link_binary_action(
    ...
    for dep in deps:
        ...
        if apple_common.Objc in dep:
            ...
            dep_dynamic_framework_names = []
            dep_dynamic_framework_paths = []
            dep_static_framework_files = []
            dep_link_flags = []

            if framework_migration.is_post_framework_migration():
                dep_dynamic_framework_names.extend(
                    objc.dynamic_framework_names.to_list(),
                )
                dep_dynamic_framework_paths.extend(
                    objc.dynamic_framework_paths.to_list(),
                )
                dep_static_framework_files.extend(
                    objc.static_framework_file.to_list(),
                )
            else:
                dep_dynamic_framework_names.extend([
            ...
            dep_link_flags.extend([
                ...
                "-F{}".format(path)
                for path in dep_dynamic_framework_paths
